...
function DailyHoursRigid(props) {
  const classes = TableStyle();
  const [data, setData] = useState(schema.dailyMinimumHoursRigid);
  ...
  const formik = useFormik({
    ...
    onSubmit: (value) => {
      const data = {
        ...value,
        truckTypeId: value?.truckTypes?.id ? value?.truckTypes?.id : 1,
      };
      delete data.id;
      ...
      dispatch({
        type: UPDATE_DAILY_MINIMUM_HOURS,
      });
      API.post(`customers/${id}/dailyMinimumHours`, newData)
        .then((response) => {
          dispatch({
            type: UPDATE_DAILY_MINIMUM_HOURS_SUCCESS,
            payload: response.data.data,
          });
          props.getDailyMinimumHours();
          toast.success("Loose Minimum Hours Updated Successfully");
        })
        .catch((error) => {
          dispatch({
            type: UPDATE_DAILY_MINIMUM_HOURS_FAILURE,
            payload: error,
          });
  ...
  useEffect(() => {
    if (state?.customer?.minimumHoursData?.length !== 0) {
      if (state?.customer?.minimumHoursData?.length === 1) {
        if (state?.customer?.minimumHoursData[0].hasOwnProperty("truckTypes")) {
          if (
            state?.customer?.minimumHoursData[0].truckTypes.name === "Rigid"
          ) {
            setData(
              state?.customer?.minimumHoursData?.find(
                (item) => item.truckTypes?.name === "Rigid"
              )
            );
          }
        }
      }
      if (state?.customer?.minimumHoursData?.length === 2) {
        setData(
          state?.customer?.minimumHoursData?.find(
            (item) => item.truckTypes?.name === "Rigid"
          )
        ...
        <div className={cx(classes.minimumHours, classes.weeklyMinimumHours)}>
          <label className="label-text">
            ...
            <span className="normal-text">
              {"  "}(Note: Values will only be charge if greater than 0)
            </span>
          </label>
              ...
              <Table stickyHeader aria-label="simple table">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell className="hours">Rigid Min. Hours</TableCell>
                    <TableCell className="amount">Rigid Rate</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  <TableRow>
...
export default DailyHoursRigid;
